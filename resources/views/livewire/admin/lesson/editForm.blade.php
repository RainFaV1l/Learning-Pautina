        ...
        <div class="admin-panel-info-new-users__container admin-crud">
            <div class="add-course__form add-course-form">
                <form wire:submit.prevent="update({{ $lesson_id }})" enctype="multipart/form-data">
                    ...
                    {{ $message }}
                    @enderror
                    <textarea wire:ignore wire:model="task" placeholder="Введите задачу..."
                              class="add-course-form__input add-course-form__textarea">{{ $task }}</textarea>
                    @error('task')
                    {{ $message }}
                    @enderror
                    <input wire:ignore value="{{ $lesson_number }}" wire:model="lesson_number" type="text"
                           class="add-course-form__input" placeholder="Номер урока">
                    @error('lesson_number')
                    {{ $message }}
                    @enderror
                    ...
                    <div class="add-course-form__img add-course-form-img dragable-img-multi lesson-edit">
                        <input wire:model="file_path" id="dragImg" type="file" name="file_path[]" multiple
                    ...
                    <div class="add-video-column video__inputs">
                        ...
                        <div class="video__input-column">
                            <input wire:model="video.0" type="text" name="inputs[0][video_path]" class="add-course-form__input"
                    ...
                    <div class="add-video-column video__inputs">
                        @foreach($counter as $key => $value)
                            ...
                            <div class="video__input-column">
                                <div class="video__input-column">
                                    <input wire:model="video.{{ $value }}" type="text" class="add-course-form__input"
                                           placeholder="Введите ссылку на видео">
                    ...
                    <div class="add-video-column video__inputs">
                        ...
                        @foreach($lesson->lessonVideos as $video)
                            <div class="video__input-column">
                                <input type="text" class="add-course-form__input"
                                       placeholder="Введите ссылку на видео" value="{{ $video->video_path }}">
                                <button wire:click.prevent="deleteVideo({{ $video }})" type="button" class="remove">Удалить</button>
                            </div>
                        @endforeach
                    </div>
                    <div class="add-course-column-button">
                        <button type="submit" data-no-turbolink="true" class="button">Сохранить</button>
                    </div>
                </form>
            </div>
        </div>
